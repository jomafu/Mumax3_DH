...
import (
	"github.com/jomafu/Mumax3_DH/httpfs"
...
func init() {
	...
	DeclFunc("Double_helix", Double_helix, "Double_helix(arm_diam, arm_separation, nm_per_turn, theta_initial, height, resolution)")
	DeclFunc("Double_helix_arm_a", Double_helix_arm_a, "(arm_diam, arm_separation, nm_per_turn, theta_initial, height, resolution)")
	DeclFunc("Double_helix_arm_b", Double_helix_arm_b, "(arm_diam, arm_separation, nm_per_turn, theta_initial, height, resolution)")
...
// Double_helix
func Double_helix(arm_diam, arm_separation, nm_per_turn, theta_initial, height, resolution float64) Shape {
	return func(x, y, z float64) bool {
		theta := theta_initial + 2*3.14159265359 * z / (nm_per_turn*1e-9)
		dx := arm_separation/2*math.Sin(theta)
		dy := arm_separation/2*math.Cos(theta)
		aux := 0
        z_aux2 := 0.0
		if z <= height/2+arm_diam/2+resolution && z >= -height/2 {
			for z_aux := z-arm_diam/2; z_aux < z+arm_diam/2; z_aux+= resolution {
                if z_aux<= height/2 {
				theta = theta_initial + 2*3.14159265359 * z_aux / (nm_per_turn*1e-9)
                z_aux2 = z_aux
                } else {
                theta = theta_initial + 2*3.14159265359 * height/2 / (nm_per_turn*1e-9)
                z_aux2 = height/2
                }
				...
				if aux == 1 {
					...
					if sqr64((x-dx)/arm_diam)+sqr64((y-dy)/arm_diam)+sqr64((z-z_aux2)/arm_diam) <= 0.25{
					aux = 1
					}
					if sqr64((x+dx)/arm_diam)+sqr64((y+dy)/arm_diam)+sqr64((z-z_aux2)/arm_diam) <= 0.25{
					aux = 1
...
// Double_helix_arm_a
func Double_helix_arm_a(arm_diam, arm_separation, nm_per_turn, theta_initial, height, resolution float64) Shape {
			return func(x, y, z float64) bool {
		theta := theta_initial + 2*3.14159265359 * z / (nm_per_turn*1e-9)
		dx := arm_separation/2*math.Sin(theta)
		dy := arm_separation/2*math.Cos(theta)
...
// Double_helix_arm_b
func Double_helix_arm_b(arm_diam, arm_separation, nm_per_turn, theta_initial, height, resolution float64) Shape {
			return func(x, y, z float64) bool {
		theta := theta_initial + 2*3.14159265359 * z / (nm_per_turn*1e-9)
		dx := arm_separation/2*math.Sin(theta)
		dy := arm_separation/2*math.Cos(theta)
